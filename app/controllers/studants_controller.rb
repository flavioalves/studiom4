class StudantsController < ApplicationController
  def create
    ...
    if studant.nil?
      ...
      render json: { status: 'arquitetura-na-pratica', message: "Olá, #{studant.name}! Obrigada pela inscrição! Você está sendo redirecionando para o PagSeguro para prosseguir com o pagamento. Clique em OK para continuar!" }
    elsif studant.studant_type == 'pacote'
      render json: { status: 'pacote', message: "Olá, #{studant.name}! Obrigada pela inscrição! Você está sendo redirecionando para o PagSeguro para prosseguir com o pagamento. Clique em OK para continuar!" }
